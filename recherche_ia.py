...
def rechercher_sur_le_web(historique, image_file=None):
    ...
    # DÉTECTION INTELLIGENTE DE L'INTENTION GRAPHIQUE
    # Si l'utilisateur fournit une image en entrée, OU s'il utilise des mots-clés de création/génération/visualisation
    mots_cles_visuels = ["génère", "crée", "dessine", "logo", "montre-moi", "illustration", "photo", "image", "fais-moi voir"]
    demande_visuelle = any(mot in derniere_requete.lower() for mot in mots_cles_visuels)
    
    # On active le mode image si l'utilisateur a joint un fichier OU si sa phrase demande explicitement une image
    is_image_mode = (image_file is not None) or demande_visuelle

    try:
        contenus_prompt = []
        
        # 1. Inclusion du contexte de conversation textuelle
        historique_texte = ""
        ...
        # 3. MODE MULTIMODAL / CRÉATION D'IMAGE INTELLIGENT
        if is_image_mode:
            if image_file is not None:
                instruction_image = (
                    f"\nCONSIGNE DE MODIFICATION : En te basant sur l'élément visuel fourni, "
                    f"recrée ou adapte la scène en respectant cette demande : {derniere_requete}."
                )
                contenus_prompt.append(instruction_image)
            else:
                instruction_creation = (
                    f"\nCONSIGNE VISUELLE : L'utilisateur souhaite voir une image ou une illustration de : '{derniere_requete}'. "
                    f"Génère une image de haute qualité représentant fidèlement cette demande."
                )
        ...
        # 4. MODE DISCUSSION & RECHERCHE SIMPLE
        else:
            response = client.models.generate_content(
        ...
        if "503" in erreur_str or "UNAVAILABLE" in erreur_str:
            message_douceur = (
                ...
                "et surchargés en moment (Erreur 503). Laissez-moi quelques secondes et relancez, je serai prête !"
            )
        else:
